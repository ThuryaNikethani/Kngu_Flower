<div align="center">
  <h1>Kngu Flower Shop 🌸</h1>
</div>

---
## 📋 Project Overview

**Kngu Flower Shop** is a beautifully designed, fully responsive flower e-commerce website developed as a **company assignment**. It showcases modern web development practices with a clean, elegant, and user-friendly interface.

This project was created after my CV was selected from the company's inquiries, demonstrating my ability to build professional, visually appealing web applications.

### ✨ Live Demo
**[View Live Website](https://knguflowersshop.vercel.app)**

---

## 🎨 Features

- **Modern & Elegant Design** with soft floral aesthetics
- **Fully Responsive** – Optimized for desktop, tablet, and mobile devices
- **Smooth Animations** and hover effects
- **Professional Product Showcase**
- **Clean Navigation** with intuitive user experience
- **Fast Loading** static website (HTML + CSS)
- **SEO-friendly** structure

---

## 🛠️ Technologies Used

- **HTML5** – Semantic markup and accessibility
- **CSS3** – Modern styling, Flexbox, Grid, and animations
- **Responsive Design** – Mobile-first approach
- **Vercel** – Deployment & Hosting

---

## 📁 Project Structure
Kngu_Flower/
├── index.html          # Main webpage
├── style.css           # All styling and animations
└── assets/             # Images and resources (if added later)

---

## 🚀 What I Learned

- Creating pixel-perfect, modern web layouts
- Implementing responsive design principles
- Writing clean, maintainable CSS
- Delivering a real client-style assignment under professional standards
- Deploying and hosting static websites using Vercel

---

## 📌 Assignment Context

This project was developed as part of a **company assignment** given after my CV was shortlisted. It reflects my dedication to delivering high-quality work and attention to detail in web development.

---
## 👤 Author
[Thurya Nikethani(Thinu)](https://github.com/ThuryaNikethani)

---
This project was developed as part of an internship for educational and professional development purposes 🌷

---
Made with 💐 & Care for Beautiful Moments
